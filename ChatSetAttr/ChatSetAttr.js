	...
	const version = "1.10",
		...
		setCharAttributes = function (charid, setting, errors, feedback, attrs, opts) {
			...
			Object.entries(attrs).forEach(([attrName, attr]) => {
				...
				notifyObservers("change", attr, oldAttr);
			});
			// Feedback
			if (!opts.silent) {
				if ("fb-content" in opts) {
					const finalFeedback = Object.entries(setting).reduce((m, [attrName, value], k) => {
						if (!charFeedback[attrName]) return m;
							...
							.replace(`_CUR${k}_`, () => htmlReplace(charFeedback[attrName].current || attrs[attrName].get("current") || ""))
							.replace(`_MAX${k}_`, () => htmlReplace(charFeedback[attrName].max || attrs[attrName].get("max") || ""));
		...
		registerObserver = function (event, observer) {
			if(observer && _.isFunction(observer) && observers.hasOwnProperty(event)) {
				...
				log("ChatSetAttr event registration unsuccessful. Please check the documentation.");
			}
		},
